...
//Create new order
router.post("/purchase/:user_id/:cartID", verifyUserToken, async (req, res) => {
  ...
  d = new Date(delivery_date)
  validMonth = d.getMonth() + 1
  delivery_date = `${d.getDate()}/${validMonth}/${d.getFullYear()}`
  todayDate = new Date()
  todayMonth = todayDate.getMonth + 1
  todayDate = `${todayDate.getDate()}/${validMonth}/${todayDate.getFullYear()}`
  console.log(todayDate)
  if (delivery_date < todayDate) return res.status(403).json({ error: true, message: 'Please enter a valid date' })
...
router.get("/latest-action/:buying_user", verifyUserToken, async (req, res) => {
...
const createOrderFile = async (cartProducts, orderData) => {
  fs.writeFileSync(`${__dirname}/order.text`, `
============================================ \n
  ORDER RECEPTION \n
  `)
  for (const key in cartProducts) {
    if (cartProducts.hasOwnProperty.call(cartProducts, key)) {
      ...
      fs.appendFileSync(`${__dirname}/order.text`,
        `
============================================
...
router.get('/reception', verifyUserToken, async (req, res) => {
  const file = `${__dirname}/order.text`
  res.download(file)
})
