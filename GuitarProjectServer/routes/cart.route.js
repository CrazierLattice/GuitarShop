...
router.post("/newcart/:userId", verifyUserToken, async (req, res) => {
  try {
...
router.post("/existing-cart/:userId", verifyUserToken, async (req, res) => {
  const { userId } = req.params;
  await cartModel
    .findOne({
      buying_user: userId,
...
router.post("/add/:userId/:productId", verifyUserToken, async (req, res) => {
  let totalPrice = 0;
...
router.delete("/delete/:userId/:productId", verifyUserToken, async (req, res) => {
  let totalPrice = 0;
  const { userId, productId } = req.params;
  const product = await guitarModel.findOne({ _id: productId });
  const activeCart = await cartModel.findOne({
  ...
  if (activeCart) {
    ...
    await cartDetailModel
      ...
      .then(async (cart) => {
        ...
        if (!cart.length) {
          await cartModel.findOneAndUpdate({ _id: activeCart._id }, { is_active: false })
        }
...
router.delete("/delete-all/:cart", verifyUserToken, async (req, res) => {
    ...
    .then(async () => {
      await cartDetailModel
        ...
        .then(async (cartDetail) => {
          await cartModel.findOneAndUpdate({ _id: cart }, { is_active: false })
            .then(() => {
              res.status(201).json({ error: false, cart: cartDetail, totalPrice: 0 });
...
router.get("/products/:userId", verifyUserToken, async (req, res) => {
