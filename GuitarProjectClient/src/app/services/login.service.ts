...
export class LoginService {
  ...
  public getTotalActions() {
    this.http.get('http://localhost:5000/login/store-data').subscribe(
      (res: any) => {
  ...
  public handleAccess() {
    this.http.post(`http://localhost:5000/login/handle-access/${this.data.user._id}`, {}, {
      headers: { 'Content-Type': 'application/json' }
    }).subscribe(
      res => {
      },
  ...
  public getUserOnRefresh(token: string) {
    this.http.get(`http://localhost:5000/login/refresh/${token}`).subscribe(
      (res: any) => {
