...
export class OrderService {
  ...
  public createNewOrder(userId, cartID, body, orderDialog) {
    this.http.post(`http://localhost:5000/order/purchase/${userId}/${cartID}`, body, {
      headers: {
  ...
  public getOrderReceptionFile(x) {
    ...
    this.http.get('http://localhost:5000/order/reception', {
      ...options, responseType: 'blob', headers: {
        'Content-Type': 'application/json',
        'xx-auth': this.data.token
      }
    }).subscribe(
